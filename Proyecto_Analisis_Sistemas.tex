...
% ===================== TIPOGRAFIA ESTILO ARIAL =====================
% pdflatex no incluye Arial real (es una fuente de Microsoft), pero Helvetica
% (via el paquete helvet) es metricamente compatible y visualmente casi
% identica -- es el sustituto estandar de Arial en LaTeX con pdflatex.
\usepackage[scaled=0.95]{helvet}
\renewcommand{\familydefault}{\sfdefault}

% ===================== PALETA DE COLOR =====================
\definecolor{brandteal}{HTML}{0E7C86}      % color principal (titulos, reglas)
\definecolor{brandtealLight}{HTML}{DCEFEF} % fondo de encabezados de tabla
\definecolor{brandamber}{HTML}{F2A93B}     % color de acento secundario

\usepackage{hyperref}
\hypersetup{colorlinks=true, linkcolor=brandteal, urlcolor=brandteal, citecolor=brandteal}

% Titulos de seccion con color y regla debajo
\titleformat{\section}
  {\normalfont\Large\bfseries\color{brandteal}}{\thesection}{0.8em}{}[{\color{brandteal!60}\titlerule[1pt]}]
\titleformat{\subsection}
  {\normalfont\large\bfseries\color{brandteal}}{\thesubsection}{0.8em}{}
...
% ===================== FIGURA DE ACTOR (muñeco UML) =====================
% Uso dentro de un tikzpicture: \actor{(x,y)}{Etiqueta}
\newcommand{\actor}[2]{%
  \begin{scope}[shift={#1}]
    \draw[thick] (0,0.55) circle (0.18);
    \draw[thick] (0,0.37) -- (0,-0.15);
    \draw[thick] (0,0.2) -- (-0.28,-0.05);
    \draw[thick] (0,0.2) -- (0.28,-0.05);
    \draw[thick] (0,-0.15) -- (-0.22,-0.55);
    \draw[thick] (0,-0.15) -- (0.22,-0.55);
    \node[align=center, font=\small, below=2pt] at (0,-0.6) {#2};
  \end{scope}
}
...
\begin{titlepage}
    ...
    % Reemplazar por: \includegraphics[width=4cm]{logo_upi.png}
    \logoplaceholder[4cm]

    \vspace{0.5cm}
    {\LARGE\bfseries\color{brandteal} Universidad\\Politécnica\\Internacional\par}

    \vspace{0.6cm}
    {\color{brandamber}\rule{6cm}{1.4pt}\par}

    \vspace{1.6cm}
    {\bfseries\large\color{brandteal} Análisis de Sistemas I\par}
...
\hline
\endhead
 & & & & 0 & \cellcolor{brandteal!25}Sin iniciar & \cellcolor{brandamber!35}Bajo & \\
\hline
 & & & & 0 & \cellcolor{brandteal!25}En curso & \cellcolor{brandamber!35}Alta & \\
\hline
 & & & & 0 & \cellcolor{brandteal!25}Completado & \cellcolor{brandamber!35}Media & \\
\hline
 & & & & 0 & \cellcolor{brandteal!25}Atrasado & \cellcolor{brandamber!35}Alta & \\
\hline
...
\begin{center}
\begin{tikzpicture}[
  box/.style={draw, rounded corners, minimum width=7cm, minimum height=1.1cm, align=center, font=\small},
  smallbox/.style={draw, rounded corners, minimum width=3.3cm, minimum height=1cm, align=center, font=\small},
  arrow/.style={-{Latex[length=2.5mm]}, thick},
  node distance=0.6cm
]
\node[box, fill=brandteal!12] (usuarios) {Usuarios\\\footnotesize Recepcionista -- Ginecólogo -- Administrador};
\node[box, fill=brandteal!22, below=of usuarios] (web) {Capa de Presentación\\\footnotesize Interfaz Web (Django + Bootstrap)};
\node[box, fill=brandteal!32, below=of web] (logica) {Capa de Lógica de Negocio\\\footnotesize Pacientes -- Citas -- Expedientes -- Permisos};
\node[box, fill=brandteal!42, below=of logica] (bd) {Capa de Datos\\\footnotesize Base de datos PostgreSQL};
\node[smallbox, fill=brandamber!25, below=1.3cm of bd, xshift=-2.1cm] (tableau) {Tableau\\\footnotesize (representar datos)};
\node[smallbox, fill=brandamber!25, below=1.3cm of bd, xshift=2.1cm] (powerbi) {Power BI / Power Apps\\\footnotesize (ingresar datos)};

\draw[arrow] (usuarios) -- (web);
\draw[arrow] (web) -- (logica);
\draw[arrow] (logica) -- (bd);
\draw[arrow] (bd.south) -- ++(0,-0.35) -| (tableau.north);
\draw[arrow] (bd.south) -- ++(0,-0.35) -| (powerbi.north);
\end{tikzpicture}

\vspace{0.2cm}
{\small\itshape Figura 1. Arquitectura del sistema GineSalud (cliente-servidor de 3 capas)}
\end{center}
...
\begin{center}
\begin{tikzpicture}[
  uc/.style={ellipse, draw, fill=brandteal!12, minimum width=5.6cm, minimum height=1cm, align=center, font=\small},
  conn/.style={thick}
]
% Limite del sistema
\draw[rounded corners, thick] (3.0,-0.9) rectangle (10.0,9.5);
\node[font=\bfseries\small] at (6.5,9.1) {Sistema GineSalud};

% Casos de uso
\node[uc] (uc1) at (6.5,8.1) {Registrar y buscar\\pacientes};
\node[uc] (uc2) at (6.5,6.75) {Gestionar citas\\médicas};
\node[uc] (uc3) at (6.5,5.4) {Consultar y actualizar\\expediente clínico};
\node[uc] (uc4) at (6.5,4.05) {Recibir recordatorios\\de citas};
\node[uc] (uc5) at (6.5,2.7) {Administrar usuarios\\y permisos};
\node[uc] (uc6) at (6.5,1.35) {Visualizar reportes\\e indicadores};
\node[uc] (uc7) at (6.5,0.0) {Gestionar catálogos\\vía Power Apps};

% Actores
\actor{(0,7.4)}{Recepcionista}
\actor{(0,5.4)}{Ginecólogo}
\actor{(13,4.0)}{Paciente}
\actor{(13,1.3)}{Administrador}

% Conexiones
\draw[conn] (0,7.4) -- (uc1.west);
\draw[conn] (0,7.4) -- (uc2.west);
\draw[conn] (0,5.4) -- (uc2.west);
\draw[conn] (0,5.4) -- (uc3.west);
\draw[conn] (13,4.0) -- (uc4.east);
\draw[conn] (13,1.3) -- (uc5.east);
\draw[conn] (13,1.3) -- (uc6.east);
\draw[conn] (13,1.3) -- (uc7.east);
\end{tikzpicture}

\vspace{0.2cm}
{\small\itshape Figura 2. Diagrama de casos de uso del sistema GineSalud}
\end{center}
...
% --- PLACEHOLDER CAPTURA TABLERO AZURE DEVOPS ---
% Reemplazar por: \includegraphics[width=0.9\textwidth]{tablero_azure_devops.png}
\placeholderimg{12}{7}{Captura del tablero Azure Boards con el Sprint actual}
...
\begin{center}
\begin{tikzpicture}[
  act/.style={draw, rounded corners, fill=brandteal!12, minimum width=6.4cm, minimum height=0.9cm, align=center, font=\small},
  side/.style={draw, rounded corners, fill=brandamber!18, minimum width=4.6cm, minimum height=0.9cm, align=center, font=\small},
  dec/.style={draw, shape=diamond, fill=white, minimum width=2.6cm, minimum height=1.3cm, align=center, font=\scriptsize, inner sep=1pt},
  lbl/.style={font=\scriptsize, fill=white, inner sep=1pt},
  arr/.style={-{Latex[length=2.2mm]}, thick}
]
\node[circle, fill=black, minimum size=0.28cm] (start) at (0,11) {};
\node[act] (a1) at (0,10) {Buscar paciente por nombre o cédula};
\node[dec] (d1) at (0,8.6) {¿Paciente\\registrada?};
\node[side] (s1) at (4.2,8.6) {Registrar nueva paciente};
\node[act] (a2) at (0,7.2) {Verificar disponibilidad del médico};
\node[dec] (d2) at (0,5.8) {¿Horario\\disponible?};
\node[side] (s2) at (4.2,5.8) {Elegir otro horario o médico};
\node[act] (a3) at (0,4.4) {Registrar la cita en el sistema};
\node[act] (a4) at (0,3.3) {Confirmar cita y notificar a la paciente};
\node[circle, draw, thick, minimum size=0.32cm] (endc) at (0,2.1) {};
\node[circle, fill=black, minimum size=0.16cm] at (0,2.1) {};

\draw[arr] (start) -- (a1);
\draw[arr] (a1) -- (d1);
\draw[arr] (d1) -- (a2) node[lbl, midway, left] {Sí};
\draw[arr] (d1) -- (s1) node[lbl, midway, above] {No};
\draw[arr] (s1) |- (a2.east);
\draw[arr] (a2) -- (d2);
\draw[arr] (d2) -- (a3) node[lbl, midway, left] {Sí};
\draw[arr] (d2) -- (s2) node[lbl, midway, above] {No};
\draw[arr] (s2) |- (a3.east);
\draw[arr] (a3) -- (a4);
\draw[arr] (a4) -- (endc);
\end{tikzpicture}
...
\vspace{0.2cm}
{\small\itshape Figura 3. Diagrama de actividad -- Registrar y confirmar cita médica}
\end{center}
...
\begin{center}
\begin{tikzpicture}[
  class/.style={rectangle split, rectangle split parts=2, draw, fill=brandtealLight, text width=2.7cm, font=\tiny, align=center},
  rel/.style={thick},
  card/.style={font=\tiny, fill=white, inner sep=1pt}
]
% Fila 1
\node[class] (rol) at (0,9.5) {\textbf{Rol}\nodepart{second}id\_rol\\nombre\_rol};
\node[class] (usuario) at (3.4,9.5) {\textbf{Usuario}\nodepart{second}id\_usuario\\nombre\_usuario\\id\_rol};
\node[class] (especialidad) at (7.4,9.5) {\textbf{Especialidad}\nodepart{second}id\_especialidad\\nombre\_especialidad};
\node[class] (medico) at (10.8,9.5) {\textbf{Medico}\nodepart{second}id\_medico\\nombre\_completo\\id\_especialidad};

% Fila 2
\node[class] (expediente) at (0,6.3) {\textbf{ExpedienteClinico}\nodepart{second}id\_expediente\\id\_paciente};
\node[class] (paciente) at (3.4,6.3) {\textbf{Paciente}\nodepart{second}id\_paciente\\nombre\_completo\\cedula};
\node[class] (cita) at (7.4,6.3) {\textbf{Cita}\nodepart{second}id\_cita\\id\_paciente\\id\_medico\\estado};

% Fila 3
\node[class] (diagnostico) at (0,3.1) {\textbf{Diagnostico}\nodepart{second}id\_diagnostico\\id\_expediente\\id\_medico};
\node[class] (tratamiento) at (3.4,3.1) {\textbf{Tratamiento}\nodepart{second}id\_tratamiento\\id\_diagnostico};

% Relaciones
\draw[rel] (usuario) -- (rol) node[card, midway, above] {N -- 1};
\draw[rel] (medico) -- (especialidad) node[card, midway, above] {N -- 1};
\draw[rel] (paciente) -- (expediente) node[card, midway, above] {1 -- 1};
\draw[rel] (cita) -- (paciente) node[card, midway, above] {N -- 1};
\draw[rel] (cita.north) -- (medico.south) node[card, midway, right] {N -- 1};
\draw[rel] (expediente) -- (diagnostico) node[card, midway, right] {1 -- N};
\draw[rel] (diagnostico) -- (tratamiento) node[card, midway, above] {1 -- N};
\end{tikzpicture}

\vspace{0.2cm}
{\small\itshape Figura 4. Diagrama de clases del sistema GineSalud}
\end{center}
...
\begin{center}
\begin{tikzpicture}[
  devnode/.style={draw, thick, rounded corners, fill=brandteal!12, minimum width=4.4cm, minimum height=1.3cm, align=center, font=\small},
  smallnode/.style={draw, thick, rounded corners, fill=brandamber!20, minimum width=3.6cm, minimum height=1.1cm, align=center, font=\small},
  arr/.style={-{Latex[length=2.2mm]}, thick}
]
\node[devnode] (cliente) at (0,7.6) {\footnotesize «dispositivo»\\\textbf{Cliente}\\Navegador web};
\node[devnode] (app) at (0,5.6) {\footnotesize «servidor»\\\textbf{Servidor de Aplicaciones}\\Django (Python)};
\node[devnode] (db) at (0,3.6) {\footnotesize «servidor»\\\textbf{Servidor de Base de Datos}\\PostgreSQL};
\node[smallnode] (tableau) at (-2.7,1.3) {\footnotesize «servicio en la nube»\\\textbf{Tableau Cloud}};
\node[smallnode] (powerbi) at (2.7,1.3) {\footnotesize «servicio en la nube»\\\textbf{Power BI / Power Apps}};

\draw[arr] (cliente) -- (app) node[midway, right, font=\scriptsize] {HTTPS};
\draw[arr] (app) -- (db) node[midway, right, font=\scriptsize] {SQL};
\draw[arr] (db.south) -- ++(0,-0.4) -| (tableau.north) node[pos=0.7, above, font=\scriptsize] {ODBC (lectura)};
\draw[arr] (db.south) -- ++(0,-0.4) -| (powerbi.north) node[pos=0.7, above, font=\scriptsize] {ODBC (lectura/escritura)};
\end{tikzpicture}

\vspace{0.2cm}
{\small\itshape Figura 5. Diagrama de despliegue del sistema GineSalud}
\end{center}
...
crítica. La Tabla 9 anterior documenta completamente la ruta crítica (tareas, duración y
...
% Reemplazar por: \includegraphics[width=0.9\textwidth]{figura6_captura_prototipo.png}
\placeholderimg{12}{7}{Figura 6. Captura del prototipo funcional (base de datos y dashboards)}
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item Mata Guerrero, J. J. (2026). \textit{Proyecto -- Análisis de Sistemas 1}
    [Instrucciones de curso]. Universidad Politécnica Internacional.
\end{itemize}
